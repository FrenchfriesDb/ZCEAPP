...
export default function ProfileScreen() {
    const { user, signOut, changeUsername, purchaseSystemBackup, completeDrill } = useUser();
    const [sharing, setSharing] = useState(false);

    const handleSocialBoost = async () => {
        setSharing(true);
        // Simulate social sharing delay
        await new Promise(r => setTimeout(r, 1500));

        // Award XP for social proof
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <GlassCard themed style={styles.heatmapCard} intensity={25}>
                    ...
                    <Pressable
                        ...
                        style={({ pressed }) => [styles.socialBoostBtn, pressed && { opacity: 0.7 }]}
                        disabled={sharing}
                    >
                        {sharing ? (
                ...
                <View style={styles.statsGrid}>
                    <GlassCard themed style={styles.statCard} intensity={15}>
                        ...
                        <Text style={[styles.statValue, { textShadowColor: 'rgba(255,255,255,0.2)' }]}>{totalDrills}</Text>
                        <Text style={[styles.statSub, { color: '#FFFFFF' }]}>REPS LOGGED</Text>
                    </GlassCard>
                    <GlassCard themed style={styles.statCard} intensity={15}>
                        <Text style={styles.statLabel}>MISSIONS</Text>
                        <Text style={[styles.statValue, { textShadowColor: 'rgba(255,255,255,0.2)' }]}>{totalMissions}</Text>
...
const styles = StyleSheet.create({
    ...
    title: { fontFamily: Fonts.monoBold, fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', opacity: 0.6 },
    levelBadge: {
